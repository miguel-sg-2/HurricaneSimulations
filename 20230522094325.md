...
Hurricanes of varying size and intensities are simulated using WRF-LES by varying surface temperature, and the initial potential temperature and relative humidity profiles.
## Initialization
...
The `namelist.input` file for each hurricane is located at `/pl/active/JKL_IDEAL/hurricanes/sst_*/initial/`.
...
An example `modify_input.py` script for each hurricane is located at `Hurricane/initialization/` or at `/pl/active/JKL_IDEAL/hurricanes/sst_*/initial/`.
...
An example `hurr_center.py` script is located at `Hurricane/postprocessing/`.
### Time series output
...
These scripts can be found here: `Hurricane/postprocessing/`.
